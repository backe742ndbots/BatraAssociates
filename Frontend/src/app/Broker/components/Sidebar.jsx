...
const MENU = [
  ...
  { to: "/broker/matches", label: "Matches", icon: "fa-building" },
  { to: "/broker/inventory", label: "Inventory & Keys", icon: "fa-key" },
  { to: "/broker/dealers", label: "Dealers & Parties", icon: "fa-handshake" },
  { to: "/broker/tasks", label: "Tasks", icon: "fa-check-square" },
  { to: "/broker/documents", label: "Documents", icon: "fa-file-alt" },
  { to: "/broker/reports", label: "Reports", icon: "fa-chart-line" },
];

export default function BrokerSidebar({ isOpen, onClose, user }) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="px-6 py-6 border-b border-slate-200 dark:border-white/10">
          ...
          <h1 className="mt-1 text-xl font-semibold tracking-wide text-slate-900 dark:text-white">
            Batra Associates
          </h1>
        </div>
    ...
    "
          >
            {/* Apple-style logout icon (SVG, always visible) */}
            <svg
