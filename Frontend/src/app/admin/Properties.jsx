...
//       <SectionHeader
//         title="Properties"
//         subtitle="Curated premium real estate listings"
//         action={
//           <Link
...
export default function Properties() {
  ...
  ========================= */
	const filteredProperties = useMemo(() => {
		...
		return properties.filter((p) => {
			// ✅ SCHEMA FIX: Use 'propertyName' only. 'propertyTitle' is ownership (Freehold/Leasehold).
			...
			const text = [
				...
				p.description,
				p.city, // Schema: city
				p.locality, // Schema: locality
				p.address, // Schema: address
				p.sector, // Schema: sector
			]
				.join(" ")
				.toLowerCase();
			...
			// Filter Logic
			// ✅ SCHEMA FIX: Check 'propertyType' (flat/plot) AND 'category' (residential)
			if (
				filters.category &&
	...
	// Helper: Safe Image URL (Handles Strings & Cloudinary Objects)
	const getImageUrl = (p) => {
		if (p.cover) {
		...
		<AdminLayout>
			<SectionHeader
				...
				action={
					<Link
						to="/admin/add-property"
						className="bg-black text-white dark:bg-white dark:text-black px-5 py-2 rounded-lg text-sm font-medium hover:opacity-90 transition"
					>
						Add Property
					</Link>
				}
					...
					{filteredProperties.map((p) => (
						...
						>
							...
							<div className="relative overflow-hidden h-52 sm:h-56">
								...
								<div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-white px-3 py-1 text-xs tracking-wide rounded-full">
									{p.city || p.locality || "Unknown City"}
								</div>
							...
							<div className="p-5 space-y-4">
								<div className="space-y-1">
									<h3 className="text-[15px] sm:text-base font-semibold text-slate-800 dark:text-slate-100 leading-snug line-clamp-1">
										{/* ✅ SCHEMA FIX: Use propertyName. Fallback to Type + City if missing */}
										{p.propertyName ||
											`${p.propertyType || "Property"} in ${p.city || "City"}`}
									</h3>
									<p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1">
										{/* ✅ SCHEMA FIX: Use address or locality */}
										{p.address || p.locality || "Address not available"}
									</p>
								</div>

								{/* ✅ SCHEMA FIX: Use priceLakhs */}
								<div className="text-lg sm:text-xl font-semibold text-slate-900 dark:text-white">
									₹ {p.priceLakhs ? Number(p.priceLakhs).toLocaleString() : "0"}{" "}
									L
								</div>

								{/* ✅ SCHEMA FIX: Use areaSqFt, bedrooms, bathrooms */}
			...
			:	<div className="px-4">
					{typeof PropertyListView !== "undefined" ?
						<PropertyListView properties={filteredProperties} />
					:	<div className="text-red-500">List View Component Missing</div>}
				</div>
			}
