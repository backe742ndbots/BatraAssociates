...
export default function Properties() {
  ...
  const filteredProperties = useMemo(() => {
    return properties.filter((p) => {
      ...
      const asking = Number(p.pricing?.askingRaw || 0);
      if (filters.minPrice && asking < Number(filters.minPrice)) return false;
      ...
      >
        ...
        <div className="relative px-6 pt-5">
          ...
          <input
            ...
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by name, code, city, dealer, remarks"
            className="
              w-full pl-12 pr-4 py-3 rounded-xl text-[13px]
              bg-slate-50/80 dark:bg-neutral-800/40
              border border-slate-200 dark:border-neutral-700
            "
          />
        ...
        <div className="px-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredProperties.map((p) => {
            ...
            return (
              <div
                key={p._id}
                className="group rounded-2xl overflow-hidden bg-white dark:bg-neutral-950 border transition-all"
              >
                {/* IMAGE */}
                <div className="relative overflow-hidden">
                  <img
                  ...
                  {p.location?.city && (
                    <div className="absolute top-4 left-4 bg-black/60 text-white px-3 py-1 text-xs rounded-full">
                      {p.location.city}
                    </div>
                  )}

                  {p.availabilityStatus && (
                    <div className="absolute bottom-4 left-4 bg-white/90 px-4 py-1 text-[11px] rounded-full">
                      {p.availabilityStatus.toUpperCase()}
                    </div>
                  )}
                </div>
                  ...
                  {(p.customFields?.AREA ||
                    ...
                    p.location?.sector) && (
                      <div className="grid grid-cols-2 gap-y-2 pt-3 border-t text-xs">
                        ...
                        {(p.bhk || p.bhkRaw) && (
                          <div>{p.bhk || p.bhkRaw} BHK</div>
                        )}
                        {p.location?.sector && (
                          <div>Sector {p.location.sector}</div>
                        )}
                      </div>
                    )}
                </div>
                ...
                <Link
                  to={`/admin/properties/${p._id}`}
                  className="block text-center py-3 text-sm font-semibold bg-slate-900 text-white hover:bg-white hover:text-black transition"
                >
                  VIEW DETAILS
                </Link>
              </div>
